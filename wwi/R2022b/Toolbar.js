...
export default class Toolbar {
  ...
  createRobotWindowButton(robotName) {
    ...
    if (typeof WbWorld.instance !== 'undefined' && WbWorld.instance.readyForUpdates) {
      WbWorld.instance.robots.forEach((robot) => {
        if (robot.window !== '<none>') {
          ...
          let robotWindow = new FloatingRobotWindow(this.parentNode, this.robotName ? this.robotName : robot.name, robotWindowUrl, robot.window, mainWindow);
          if (mainWindow)
            this.robotWindows.unshift(robotWindow);
          else
            this.robotWindows.push(robotWindow);
          this._addRobotWindowToPane(this.robotName ? this.robotName : robot.name, mainWindow);
        }
      });
